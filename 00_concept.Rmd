...
for(i in 1:nrow(lr_tbl)) {
  predict_fn(lr_tbl %>% select(학습시간) %>% slice(i), lr_opt[, 1:2])
}

predict_fn(lr_tbl$학습시간, lr_opt[, 1:2])

ncol(lr_tbl %>% select(학습시간) )

length(lr_opt[, 1:2])

predict_fn(lr_tbl %>% select(학습시간) %>% slice(2), lr_opt[, 1:2])

lr_tbl_pred <- lr_tbl %>% select(-입학여부) %>% 
  mutate(par = lr_opt[, 1:2])

lr_tbl_pred %>% 
  mutate(pred = map2(학습시간, par, predict_fn) )
