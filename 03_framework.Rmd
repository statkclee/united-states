...
[케라스(Keras)](https://keras.io/)는 플러그-앤-플레이(Plug&Play) 방식으로 사용자 관점에서는 쉽게 딥러닝 모형을 개발을 지원하고 **구글(Google)**에서 이를 운영계로 서비스를 원활히 지원한다. 하지만, 각 블록이 블랙박스 형태로 되어 있다보니 이런 아쉬운 점을 보완해주는 **페이스북(Facebook)**이 지원하는 [파이토치(Pytorch)](https://pytorch.org/)가 수학기반으로 저수준(low-level) 개발을 원하는 사용자에게 선택지를 넓혀졌다는 평가를 받고 있다.
